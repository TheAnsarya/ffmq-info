...
class TextExtractor:
	...
	def save_json(self, output_dir: str, text_data: Dict):
		"""Save extracted text to JSON format"""
		output_path = Path(output_dir)
		output_path.mkdir(parents=True, exist_ok=True)
		...
		json_output = {
			...
			'extraction_date': None,  # Will be set by caller if needed
			'total_strings': sum(len(strings) for strings in text_data.values()),
			'tables': {}
		}
		
		for table_name, strings in text_data.items():
			json_output['tables'][table_name] = {
				'count': len(strings),
				'config': self.TEXT_TABLES.get(table_name, {}),
				'strings': strings
			}
		
		# Save to file
		json_file = output_path / 'text_data.json'
		with open(json_file, 'w', encoding='utf-8') as f:
			json.dump(json_output, f, indent=2, ensure_ascii=False)
		
		print(f"Saved JSON: {json_file}")
		
	def save_csv(self, output_dir: str, text_data: Dict):
		"""Save extracted text to CSV format (all tables combined)"""
		output_path = Path(output_dir)
		output_path.mkdir(parents=True, exist_ok=True)
		
		csv_file = output_path / 'text_data.csv'
		
		with open(csv_file, 'w', encoding='utf-8', newline='') as f:
			writer = csv.writer(f)
			
			# Header
			writer.writerow(['Table', 'ID', 'Text', 'Address', 'Length', 'Pointer'])
			...
			for table_name, strings in text_data.items():
				for entry in strings:
					writer.writerow([
						...
						entry['address'],
						entry['length'],
						entry.get('pointer', '')  # Pointer only exists for dialog
					])
		
		print(f"Saved CSV: {csv_file}")
		
	def save_po_file(self, output_dir: str, text_data: Dict):
		"""Save extracted text to PO format for translation"""
		output_path = Path(output_dir)
		output_path.mkdir(parents=True, exist_ok=True)
		
		po_file = output_path / 'ffmq_text.po'
		
		with open(po_file, 'w', encoding='utf-8') as f:
			# PO file header
			f.write('# Final Fantasy Mystic Quest - Translation Template\n')
			f.write('# Generated by extract_text.py\n')
			f.write('msgid ""\n')
			f.write('msgstr ""\n')
			...
			for table_name, strings in text_data.items():
				...
				for entry in strings:
					# Context comment
					f.write(f'#: {table_name}:{entry["id"]:04d} @ {entry["address"]}\n')
					
					# Source text (msgid)
					text_escaped = entry['text'].replace('"', '\\"').replace('\n', '\\n')
					f.write(f'msgid "{text_escaped}"\n')
					
					# Translation (msgstr) - empty for template
					f.write('msgstr ""\n')
					f.write('\n')
		
		print(f"Saved PO file: {po_file}")
		
	def generate_statistics(self, text_data: Dict) -> Dict:
		...
		for table_name, strings in text_data.items():
			...
			if strings:
				table_stats['avg_length'] = table_stats['total_length'] / len(strings)
				table_stats['max_length'] = max(s['length'] for s in strings)
				table_stats['min_length'] = min(s['length'] for s in strings)
			
			# Count control codes
			for entry in strings:
				text = entry['text']
				stats['control_code_usage']['newline'] += text.count('\n')
				stats['control_code_usage']['wait'] += text.count('[WAIT]')
				stats['control_code_usage']['clear'] += text.count('[CLEAR]')
				stats['control_code_usage']['name'] += text.count('[NAME]')
				stats['control_code_usage']['item'] += text.count('[ITEM]')
			
			stats['tables'][table_name] = table_stats
			stats['total_strings'] += len(strings)
			stats['total_bytes'] += table_stats['total_length']
		
		return stats
	
	def save_statistics_report(self, output_dir: str, text_data: Dict):
		"""Save detailed statistics report"""
		output_path = Path(output_dir)
		output_path.mkdir(parents=True, exist_ok=True)
		
		stats = self.generate_statistics(text_data)
		
		report_file = output_path / 'text_statistics.txt'
		...
		with open(report_file, 'w', encoding='utf-8') as f:
			...
			for code, count in stats['control_code_usage'].items():
				f.write(f"  {code.upper():10s}: {count:5d}\n")
			f.write("\n")
			...
			for table_name, table_stats in stats['tables'].items():
				...
				f.write(f"  Max length: {table_stats['max_length']:4d} bytes\n")
			
			f.write("\n" + "="*80 + "\n")
		
		print(f"Saved statistics: {report_file}")
...
def main():
	...
	if len(sys.argv) < 3:
		...
		print("  - JSON file (text_data.json) - Structured data")
		print("  - CSV file (text_data.csv) - Spreadsheet format")
		print("  - PO file (ffmq_text.po) - Translation template")
		print("  - Statistics (text_statistics.txt) - Analysis report")
		sys.exit(1)
	...
	if 'all' in output_formats:
		output_formats = ['txt', 'asm', 'json', 'csv', 'po']
	...
	if 'json' in output_formats:
		print("\nSaving JSON...")
		extractor.save_json(output_dir, text_data)
	
	if 'csv' in output_formats:
		print("\nSaving CSV...")
		extractor.save_csv(output_dir, text_data)
	
	if 'po' in output_formats:
		print("\nSaving PO file...")
		extractor.save_po_file(output_dir, text_data)
	
	if show_stats or 'all' in sys.argv:
		print("\nGenerating statistics...")
		extractor.save_statistics_report(output_dir, text_data)
