...
class GameEditor:
	...
	def __init__(self, width: int = 1400, height: int = 900):
		...
		self.graphics_db = GraphicsDatabase()
	...
	def load_rom(self, rom_path: str):
		...
		try:
			...
			self.status_bar.set_message("Loading graphics...", COLOR_TEXT)
			self.graphics_db.load_from_rom(rom_path)

			self.status_bar.set_message("Loading music...", COLOR_TEXT)
			self.music_db.load_from_rom(rom_path)
	...
	def update_stats(self):
		...
		self.stats_data = {
			...
			'tilesets': len(self.graphics_db.tilesets),
			'palettes': len(self.graphics_db.palettes),
			'tracks': len(self.music_db.tracks),
			'sfx': len(self.music_db.sfx),
		}
	...
	def export_data(self):
		...
		try:
			...
			elif self.current_tab == 6:  # Graphics
				if self.graphics_db.tilesets:
					tileset_id = 0
					self.graphics_db.export_tileset_to_image(tileset_id, 0, f"export_tileset_{tileset_id}.png")
					self.status_bar.set_message(f"Tileset {tileset_id} exported to PNG", COLOR_SUCCESS)
				else:
					self.status_bar.set_message("No tilesets to export", COLOR_WARNING)
			elif self.current_tab == 7:  # Music
				if self.music_db.tracks:
					track_id = list(self.music_db.tracks.keys())[0]
					self.music_db.export_spc(track_id, f"export_track_{track_id:02X}.spc")
					self.status_bar.set_message(f"Track {track_id:02X} exported to SPC", COLOR_SUCCESS)
				else:
					self.status_bar.set_message("No tracks to export", COLOR_WARNING)
			else:
	...
	def draw_content(self):
		...
		if self.show_stats and self.rom_loaded:
			...
			if self.current_tab == 1:  # Dialogs
				...
				gfx_stats = self.graphics_db.get_statistics()
				stats_to_show = [
					f"Tilesets: {gfx_stats.get('total_tilesets', 0)}",
					f"Palettes: {gfx_stats.get('total_palettes', 0)}",
					f"Total Tiles: {gfx_stats.get('total_tiles', 0)}",
					f"Character Palettes: {gfx_stats.get('character_palettes', 0)}",
					f"Enemy Palettes: {gfx_stats.get('enemy_palettes', 0)}"
				]
			elif self.current_tab == 7:  # Music
				music_stats = self.music_db.get_statistics()
				stats_to_show = [
					f"Music Tracks: {music_stats.get('total_tracks', 0)}",
					f"Sound Effects: {music_stats.get('total_sfx', 0)}",
					f"Samples: {music_stats.get('total_samples', 0)}",
					f"Music Data: {music_stats.get('total_music_size', 0)} bytes",
					f"SFX Data: {music_stats.get('total_sfx_size', 0)} bytes"
				]
